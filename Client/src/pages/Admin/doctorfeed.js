...
const AdminOrders = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await fetch("http://localhost:8080/alldoctors", {
        credentials: "include",
      });
    ...
    const storedAcceptedDoctors =
      JSON.parse(localStorage.getItem("acceptedDoctor")) || [];
    const storedRejectedDoctors =
      JSON.parse(localStorage.getItem("rejectedDoctor")) || [];
    setAcceptedDoctors(storedAcceptedDoctors);
    setRejectedDoctors(storedRejectedDoctors);
  }, []);

  useEffect(() => {
    localStorage.setItem("acceptedDoctor", JSON.stringify(acceptedDoctors));
    localStorage.setItem("rejectedDoctor", JSON.stringify(rejectedDoctors));
    ...
    if (
      ...
      try {
        const response = await fetch(
          `http://localhost:8080/updateStatus/${doctorToUpdate._id}`,
          {
            method: "PUT",
            headers: {
